    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            else
            {
                ...
                ESP_LOGI(TAG, "Sending to UdpQueue at %i", message.timeStamp);
                xQueueSend(xUdpQueue, &message, pdMS_TO_TICKS(0));
                xQueueSend(xUdpReturnQueue, &return_message, pdMS_TO_TICKS(0));
...
// it will wait for data to be received in a queue
void send_Data()
{
